...
def list_study_files() -> list[str]:
    ...
    Returns
        list[str]: a list of filenames relative to the notes directory.
    ...
    if not NOTES_BASE.exists():
        return []
    return sorted([str(f.relative_to(NOTES_BASE)) for f in NOTES_BASE.rglob("*.md")])


@mcp.tool()
def read_study_file(filename: str) -> str:
    """
    Reads the full content of a study note file.

    Args:
        filename (str): The filename to read, exactly as returned by 
        list_study_files(). Example: 'closures.md'

    Returns:
        str: The full text content, or an error string if not found.
...
def get_notes_index() -> str:
    """ 
        Resources: index of all available study materials with file sizes.
        URI: notes://index
    """
